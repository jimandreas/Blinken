...
v1.4: notification listening, per-app allowlist with color/duration, the lockscreen flash, repeat-until-dismissed reminders, the continuous charging-mode snake display, and an in-app About screen (source link, version) are implemented. The snake display now hides the status and navigation bars for a true full-screen sweep (swipe from an edge to reveal them briefly). Signed release builds are published automatically to [GitHub Releases](https://github.com/jimandreas/Blinken/releases) whenever a `vX.Y` tag is pushed (see `.github/workflows/release.yml`). Not yet built: a foreground service to improve reliability against aggressive OEM background-process killing (e.g. Sony's Stamina mode) — for now, the app requests a battery-optimization exemption instead, and reminders use `AlarmManager` rather than a foreground service. See `CLAUDE.md` for architecture notes and `app-suggestions.txt` for the original design discussion.
